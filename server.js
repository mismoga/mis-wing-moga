const ALLOWED_CHANGE_TYPES = ["Class Change","Gender Change","Name change"];
...
app.get("/api/records", async (req, res) => {
  try {
    ...
    for (const row of result.rows) {
      ...
      row.bmis_status = String(row.bmis_remarks || "").trim()
        ? (row.document_path ? "READY_TO_DOWNLOAD_UPDATE" : "UPDATED_BY_BMIS")
        : "PENDING_BMIS";
    }
...
const udiseSafe = String(p.udise_code || "").replace(/[^a-zA-Z0-9_-]/g, "");
    ...
    if (up.error) return res.status(500).json({error:"Document upload failed: "+up.error.message});
      ...
      [
        p.block_name || "", p.school_name || "", p.udise_code || "",
        p.pen_number || "", p.student_name || "", p.new_class || "",
        p.new_section || "", p.new_gender || "", normalizedChangeType,
        p.bmis_remarks || "", filePath || ""
      ]
    );
    res.json({ok:true});
  } catch(e) {
...
app.put("/api/records/:id", adminOnly, async (req,res) => {
  try {
    ...
    const bmisRemarks = String(p.bmis_remarks || "").trim();
    if (!bmisRemarks)
      return res.status(400).json({error:"BMIS Remarks are required before updating the record."});
    ...
    const updateResult = await pool.query(
      `UPDATE students SET
       ...
       change_type=$9, bmis_remarks=$10, updated_at=NOW()
       WHERE id=$11`,
      [
        p.block_name || "", p.school_name || "", udise, pen,
        p.student_name || "", p.new_class || "", p.new_section || "",
        p.new_gender || "", updateChangeType, bmisRemarks, id
      ]
    );
